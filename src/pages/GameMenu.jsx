...
export function GameMenu() {
  ...
  return (
    <div className="min-h-screen bg-cover bg-center" style={{ backgroundImage: "url('/background/mb.jpg')" }}>
      <div className="bg-black/60 min-h-screen backdrop-blur-sm">
        <div className="relative z-10">
          <InfoBar />
        </div>

        <div className="max-w-6xl mx-auto px-4 py-10 flex flex-col gap-8 mt-6">
          <button
            onClick={() => navigate('/')}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
            <aside className="bg-white/10 border border-white/20 rounded-3xl p-4 flex flex-col gap-3">
              {locations.map(location => (
                <button
                  key={location.id}
                  onClick={() => setFocusedLocationId(location.id)}
                  className={`text-left rounded-2xl px-4 py-3 transition font-semibold text-white ${
